...
const notificationObserver = new Observer<
  (IOpenNotification & { type: "success" | "error" })[]
>(null);
...
export const open = (options: IOpenNotification) => {
  notificationObserver.update([...(notificationObserver.value ?? []), options]);
  setTimeout(() => {
    notificationObserver.update(
      notificationObserver.value.filter((x) => x !== options)
    );
...
const icons = {
  success: (
    <span className={styles["icon-success"]}>
      <svg
  ...
  error: (
    <span className={styles["icon-error"]}>
      <svg
        viewBox="64 64 896 896"
        focusable="false"
...
notificationObserver.listen((notifications) => {
  ReactDOM.render(
...
function Notification({ closeable, description, title, icon }: INotification) {
  return (
